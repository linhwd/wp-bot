...
def account_access_status(account: dict) -> dict:
    """
    Trả về {active: bool, source: 'trial'|'subscription'|'admin'|None, expires_at: datetime|None}
    """
    # Admin luôn có quyền dùng không giới hạn, không cần mua gói cho chính mình.
    if account.get("is_admin"):
        return {"active": True, "source": "admin", "expires_at": datetime(2099, 12, 31)}
